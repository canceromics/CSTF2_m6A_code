...
ggtopo_for_count=function(anno_table){
  ...
  anno_table[which(anno_table$Gene.site=="CDS"),'Peak.position']=anno_table[which(anno_table$Gene.site=="CDS"),'Peak.position']+100
  anno_table[which(anno_table$Gene.site=="3UTR"),'Peak.position']=anno_table[which(anno_table$Gene.site=="3UTR"),'Peak.position']+200
  ggplot(anno_table)+geom_line(aes(x=anno_table$Peak.position,color=anno_table$type),stat = "count",size=1)+
    geom_vline(aes(xintercept=100), colour="#40004B", linetype="dashed",size=1)+
    geom_vline(aes(xintercept=200), colour="#40004B", linetype="dashed",size=1)+
...
specific_peak_survival=function(peak_for_analysis,peak_freq,clinic_data_for_analysis,tumor_sample_in_peak){
  ...
  surv=Surv(time = clinic_data_for_analysis$PFS_month,clinic_data_for_analysis$zhuanyi.fufa==1)
  result=c()
  while(i<=n){
...
specific_peak_survival_for_common_hyper=function(peak_for_analysis,clinic_data_for_analysis,tumor_sample_in_peak,peak_matrix){
  ...
  rownames(clinic_data_for_analysis)=clinic_data_for_analysis$t_id
  clinic_data_for_analysis=clinic_data_for_analysis[gsub("X","",tumor_sample_in_peak),]
  surv=Surv(time = clinic_data_for_analysis$PFS_month,clinic_data_for_analysis$PFS_Status==1)
  result=c()
  while(i<=n){
    pvalue=c()
...
specific_peak_survival_for_common_OS=function(peak_for_analysis,clinic_data_for_analysis,tumor_sample_in_peak,peak_matrix){
  i=1
  n=length(peak_for_analysis)
  rownames(clinic_data_for_analysis)=clinic_data_for_analysis$t_id
  clinic_data_for_analysis=clinic_data_for_analysis[gsub("X","",tumor_sample_in_peak),]
...
build_RF_script=function(gene_matrix,
  ...
  while(j<=k){
    ...
    temp.script=paste("sed 's/xingyang_data/[",paste(data_in_script,collapse = ","),
                      "]/g' /data/xingyang/m6A_zhengjian/py.test/py.script/Construct_model_and_output.py |sed 's/xingyang_target/[",
                      paste(target.temp,collapse = ","),
                      "]/g'|sed 's/feature_list_xingyang/[",paste("\"target.gene.",target.gene,"\"",collapse = ",",sep=""),
                      ",\"gender\",\"age\",\"smoke\",\"drink\",\"fenhua_class\",\"nerve\",\"xueguan\",\"node\",\"TNM_3\"]/g'|sed 's/temp_dir_xingyang/",
                      gsub("-","_",gsub(":","_",peak.temp)),"_temp/g' >  /data/xingyang/m6A_zhengjian/py.test/py.script/combine_temp/",
...
run_RF=function(clinic_data.for.analysis,target.peak.matrix,gene_matrix,target.gene,Clinic.factor.temp){
  ...
  write.table(get_script,"/data/xingyang/m6A_zhengjian/py.test/py.script/combine_temp/get_script.sh",
  ...
  #grep "[" for pvalue
  #################################################################
  system("sh /data/xingyang/m6A_zhengjian/py.test/py.script/combine_temp/get_script.sh")
  system("ls /data/xingyang/m6A_zhengjian/py.test/py.script/combine_temp/*_Construct_model_and_output.py|awk \'{print \"python3 \" $1\" > \"$1\".result\"}\'|xargs -iCMD -P60 bash -c CMD")
  system("grep {  /data/xingyang/m6A_zhengjian/py.test/py.script/combine_temp/*_Construct_model_and_output.py.result > /data/xingyang/m6A_zhengjian/py.test/py.script/combine_temp/WER.contribution.randomforest.txt")
  #system("grep \"^\\[\" /data/xingyang/m6A_zhengjian/py.test/py.script/combine_temp/*_Construct_model_and_output.py.result > /data/xingyang/m6A_zhengjian/py.test/py.script/combine_temp/WER.pvalue.randomforest.txt")
  system("rm -r /data/xingyang/m6A_zhengjian/py.test/py.script/combine_temp/*_Construct_model_and_output.py*")
  
  #random forest contribution
  contribution=read.table("/data/xingyang/m6A_zhengjian/py.test/py.script/combine_temp/WER.contribution.randomforest.txt",sep=",")
  ...
  # RF_p=read.table("/data/xingyang/m6A_zhengjian/py.test/py.script/combine_temp/WER.pvalue.randomforest.txt",sep=",",fill=T)
  # 
...
run_RF2=function(clinic_data.for.analysis,
                 target.peak.matrix,
                 gene_matrix,
                 target.gene,
                 Clinic.factor.temp){
  get_script=build_RF_script2(clinic_data.for.analysis=clinic_data.for.analysis,
...
extractLargeScaleEvents_fix=function (final.objects, thr = 0.5) 
{
  ...
  chrs <- as.vector(sapply(c(1:22,"X"), function(x) c(paste(x, "p", 
                                                     sep = ""), paste(x, "q", sep = ""))))
  finalChrMat <- matrix(0, ncol = 46, nrow = length(rownames(mergeScales$mergeScalesAmp)))
...
readBAFExtractOutput_fix=function (path, sequencing.type = "bulk", suffix = "snp") 
{
  files <- list.files(path)
  files <- files[grep(paste0(".", suffix, "$"), files)]
  loh <- list()
  for (i in 1:length(files)) {
    maf <- read.table(file.path(path, files[i]), header = F, 
                      stringsAsFactor = F)
    maf <- data.frame(chr = maf[, 1], position = maf[, 2], 
                      alt = maf[, 5], ref = maf[, 6] - maf[, 5], coverage = maf[,6], 
                      baf = maf[, 5]/maf[, 6], dev = abs(as.numeric(maf[,5]/maf[, 6]) - 0.5))
    maf <- maf[order(maf$position), ]
    idx = unlist(as.vector((sapply(c(1:22,"X"), function(x) as.vector(unlist(which(as.character(maf$chr) == 
                                                                              x)))))))
    maf <- maf[idx, ]
    j <- 0.2
...
generateAnnotation_fix=function (id_type = "ensembl_gene_id", genes, ishg19, centromere, 
          host = "www.ensembl.org") 
{
  ...
  chr.list <- as.character(c(1:22, "X"))
  ...
  colnames(annotation)[c(1:5, 7)] <- c("Gene", "GeneSymbol", 
                                       "Chr", "start", "end", "cytoband")
  annotation$isCentromer <- rep("no", nrow(annotation))
  centromere_snps <- NULL
  for (k in 1:(dim(centromere)[1])) {
    annotation$isCentromer[which(as.character(annotation$Chr) == 
                                   gsub("chr", "", as.character(centromere$V1[k])) & 
                                   (as.numeric(as.character(annotation$Position)) >= 
                                      centromere$V2[k] & as.numeric(as.character(annotation$Position)) <= 
                                      centromere$V3[k]))] <- "yes"
  }
